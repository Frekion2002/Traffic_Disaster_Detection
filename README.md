...
## 실제 CCTV 영상 테스트 방법
 ...
 고속도로, 시내도로 교통관제용 CCTV는 15 ~ 30fps로 촬영되는 경우가 많기 때문에 15프레임 단위로 탐지를 수행합니다. 또한 잘못된 정보 전달을 방지하기 위해 15프레임씩 탐지한 결과 연속 5번 이상 같은 class의 재난이 탐지가 되어야 해당 class에 속하는 재난이 발생했다고 판단하도록 합니다.


홍수 영상과 씽크홀, 화제는 유튜브에 있는 CCTV 영상으로 실제 테스트를 진행하였고, 아무런 재난 상황이 발생하지 않은 도로는 직접 핸드폰 카메라로 촬영 후 테스트를 진행하였습니다. 
